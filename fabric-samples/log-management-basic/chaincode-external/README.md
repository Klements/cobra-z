# Log-Transfer-Basic as an external service
...
```
externalBuilders:
    - path: /opt/gopath/src/github.com/hyperledger/fabric-samples/log-transfer-basic/chaincode-external/sampleBuilder
...
The Log-Transfer-Basic external chaincode requires two environment variables to run, `CHAINCODE_SERVER_ADDRESS` and `CHAINCODE_ID`, which are described and set in the `chaincode.env` file.

You need to provide a `connection.json` configuration file to your peer in order to connect to the external Log-Transfer-Basic service. The address specified in the `connection.json` must correspond to the `CHAINCODE_SERVER_ADDRESS` value in `chaincode.env`, which is `log-transfer-basic.org1.example.com:9999` in our example.
...
Open a new terminal and navigate to the `fabric-samples/log-transfer-basic/chaincode-external` directory.

```
cd fabric-samples/log-transfer-basic/chaincode-external
```
...
```
tar cfz log-transfer-basic-external.tgz metadata.json code.tar.gz
```
...
Run the following commands to install the `log-transfer-basic-external.tar.gz` chaincode on org1. The `setGlobals` function simply sets the environment variables that allow you to act as org1 or org2.

```
setGlobals 1
peer lifecycle chaincode install ../log-transfer-basic/chaincode-external/log-transfer-basic-external.tgz
```
...
## Running the Log-Transfer-Basic external service
...
After you edit the `chaincode.env` file, you can use the `Dockerfile` to build an image of the external Log-Transfer-Basic chaincode:

```
docker build -t hyperledger/log-transfer-basic .
```

You can then run the image to start the Log-Transfer-Basic service:

```
docker run -it --rm --name log-transfer-basic.org1.example.com --hostname log-transfer-basic.org1.example.com --env-file chaincode.env --network=fabric_test hyperledger/log-transfer-basic
```

This will start and run the external chaincode service within the container.

## Deploy the Log-Transfer-Basic external chaincode definition to the channel
...
## Using the Log-Transfer-Basic external chaincode

Open yet another terminal and navigate to the `fabric-samples/log-transfer-basic/application-gateway-go` directory:

```
cd fabric-samples/log-transfer-basic/application-gateway-go
```
...
- As a first step generate a keypair that can be used. Run these commands from the `fabric-samples/log-transfer-basic/chaincode-external` directory.
...
```
openssl req -nodes -x509 -newkey rsa:4096 -keyout crypto/key1.pem -out crypto/cert1.pem -subj "/C=IN/ST=KA/L=Bangalore/O=example Inc/OU=Developer/CN=log-transfer-basic.org1.example.com/emailAddress=dev@log-transfer-basic.org1.example.com"
```

For `org2.example.com`

```
openssl req -nodes -x509 -newkey rsa:4096 -keyout crypto/key2.pem -out crypto/cert2.pem -subj "/C=IN/ST=KA/L=Bangalore/O=example Inc/OU=Developer/CN=log-transfer-basic.org2.example.com/emailAddress=dev@log-transfer-basic.org2.example.com"
```
...
```
{
  "address": "log-transfer-basic.org1.example.com:9999",
  "dial_timeout": "10s",
  "tls_required": true,
...
- Follow the instructions in [Package](#packaging-and-installing-chaincode) and [Install](#installing-the-external-chaincode) steps for each organization. Remember that the chaincode server's address for the second organization is `log-transfer-basic.org2.example.com:9999`.
...
- Follow the instructions in [Finish Deployment](#finish-deploying-the-log-transfer-basic-external-chaincode-) for each organization seperately.
