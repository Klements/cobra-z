# Log Transfer REST API Sample

Sample REST server to demonstrate good Fabric Node SDK practices.

The REST API is only intended to work with the [basic log transfer example](https://github.com/hyperledger/fabric-samples/tree/main/log-transfer-basic).

To install the basic log transfer chaincode on a local Fabric network, follow the [Using the Fabric test network](https://hyperledger-fabric.readthedocs.io/en/release-2.4/test_network.html) tutorial. You need to go at least as far as the step where the ledger gets initialized with logs.
...
While the basic log transfer chaincode maps well to an `/api/logs` REST API, response times when submitting transactions to a Fabric network are problematic for REST APIs.
...
Jobs are used for submitting transactions to create, update, delete, or transfer an log.
...
Jobs are not used to get logs, because evaluating transactions is typically much faster.

Related files:
- [src/logs.router.ts](src/logs.router.ts)
  Defines the main `/api/logs` endpoint.
- [src/fabric.ts](src/fabric.ts)
  All the sample code which interacts with the Fabric network via the Fabric SDK.
...
Clone the `fabric-samples` repository and change to the `fabric-samples/log-transfer-basic/rest-api-typescript` directory before running the following commands
...
If everything went well, you can now open a new terminal and try out some basic log transfer REST calls!
...
```shell
curl --header "X-Api-Key: ${SAMPLE_APIKEY}" http://localhost:3000/api/logs
```

You should see all the available logs, for example

```
[{"AppraisedValue":300,"Color":"blue","ID":"asset1","Owner":"Tomoko","Size":5},{"AppraisedValue":400,"Color":"red","ID":"asset2","Owner":"Brad","Size":5},{"AppraisedValue":500,"Color":"green","ID":"asset3","Owner":"Jin Soo","Size":10},{"AppraisedValue":600,"Color":"yellow","ID":"asset4","Owner":"Max","Size":10},{"AppraisedValue":700,"Color":"black","ID":"asset5","Owner":"Adriana","Size":15},{"AppraisedValue":800,"Color":"white","ID":"asset6","Owner":"Michel","Size":15}]
```

### Check whether an log exists...

```shell
curl --include --header "X-Api-Key: ${SAMPLE_APIKEY}" --request OPTIONS http://localhost:3000/api/logs/asset7
```

### Create an log...

```shell
curl --include --header "Content-Type: application/json" --header "X-Api-Key: ${SAMPLE_APIKEY}" --request POST --data '{"ID":"asset7","Color":"red","Size":42,"Owner":"Jean","AppraisedValue":101}' http://localhost:3000/api/logs
```
...
```shell
curl --header "X-Api-Key: ${SAMPLE_APIKEY}" http://localhost:3000/api/logs/asset7
```

You should see the newly created log, for example
...
```shell
curl --include --header "Content-Type: application/json" --header "X-Api-Key: ${SAMPLE_APIKEY}" --request PUT --data '{"ID":"asset7","Color":"red","Size":11,"Owner":"Jean","AppraisedValue":101}' http://localhost:3000/api/logs/asset7
```

### Transfer an log...

```shell
curl --include --header "Content-Type: application/json" --header "X-Api-Key: ${SAMPLE_APIKEY}" --request PATCH --data '[{"op":"replace","path":"/Owner","value":"Ashleigh"}]' http://localhost:3000/api/logs/asset7
```

### Delete an log...

```shell
curl --include --header "X-Api-Key: ${SAMPLE_APIKEY}" --request DELETE http://localhost:3000/api/logs/asset7
```
